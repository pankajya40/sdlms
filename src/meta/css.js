...
const buildImports = {
	...
	mobile: function (source) {
		return source + '\n' + [
			'@import "../../public/less/mobile";',
		].map(function (str) {
			return str.replace(/\//g, path.sep);
		}).join('\n');
	},
...
async function getBundleMetadata(target) {
	...
	if(target == 'client') mobile =  buildImports.mobile(imports);
	

	return { paths: paths, imports: imports ,mobile:mobile};
}
/**
 * @author Unknown
 * @date   06-02-2021
...
CSS.buildBundle = async function (target, fork) {
	...
	if(data.mobile){
		await rimrafAsync(path.join(__dirname, '../../build/public/mobile*'));
		const bundle = await minifier.css.bundle(data.mobile, data.paths, minify, fork);
		await fs.promises.writeFile(path.join(__dirname, '../../build/public', 'mobile.css'), bundle.code);
	}
...
//		stylesheet: req.url.startsWith('/mobile/') ? 'mobile' : 'styles',
//<link rel="stylesheet" type="text/css" href="{relative_path}/assets/{config.stylesheet}.css?{config.cache-buster}" />
