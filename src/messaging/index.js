...
async function getAllMessages(roomId,limit,page)
{
	const collectionName = db.collections.DEFAULT;
    const keys = { _key:{regex:/^message:*/},roomId:`${roomId}` };
	const data = await db.getFieldsWithPagination(collectionName,keys,limit,0,{timestamp:-1});
	return data;

}

Messaging.getAllMessages = async(roomId,limit,page,uid) =>
{
	const isNew = false;
	const indices = {}
	const collectionName = db.collections.DEFAULT;
	...
	const data = await db.getFieldsWithPagination(collectionName, keys,50,0,{timestamp:-1});

	let mids = [];
	(data.forEach((msgData) => {
		const val = msgData._key.slice(8);
		mids.push(val)
	}))
	mids.reverse()
