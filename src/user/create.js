...
module.exports = function (User) {
	...
	async function create(data) {
		...
		const customFields = ['role', 'signature', 'personality_traits', 'favourite_authors', 'favourite_movies', 'followed', 'uploadedpicture', 'personal_assets_id', 'picture'];
		...
		[...customFields, 'picture', 'fullname', 'location', 'birthday'].forEach((field) => {
			if (data[field]) {
		...
		let userType = '';
		if (data.email) {
			let payload = await db.findField(db.collections.DEFAULT, { _key: `invitation:email:${data.email}`})
			if (payload && payload.userType) userType = payload.userType;
		}
		...
		await Promise.all([
			...
			db.updateField(db.collections.DEFAULT, { _key: `user:${uid}` }, {$set: {userType}}, {upsert: false})
		]);
