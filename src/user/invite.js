...
module.exports = function (User) {
	...
	User.sendInvitationEmail = async function (uid, email, groupsToJoin, userType) {
		if (!uid) {
			throw new Error('[[error:invalid-uid]]');
		}

		if (userType) {
			if (!SDLMS.userTypes.includes(userType)) {
		...
		const data = await prepareInvitation(uid, email, groupsToJoin, userType);
	...
	async function prepareInvitation(uid, email, groupsToJoin, userType) {
		...
		const { username, fullname } = await User.getUserFields(uid, [
			"username",
			"fullname",
		]);
