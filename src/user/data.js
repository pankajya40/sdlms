...
module.exports = function (User) {
	...
	User.getUsersFields = async function (uids, fields) {
		...
		if (users.length && users[0].uid) {
			/**
			 * @author imshawan {27-06-2022}
			 * @description Users who doesnot have a profile picture will get an auto-assigned default profile picture
			 */

			if (users[0].hasOwnProperty('picture')) {
				let { picture, uid } = users[0];
				if (!picture) {
					let usr = await db.findField(db.collections.DEFAULT, { _key: `user:${uid}` });
					if (!usr.picture) {
						let { defaultProfileImagesBase, defaultProfileImages } = constants;
						let image = `${defaultProfileImagesBase}/${
							defaultProfileImages[
								Math.floor(Math.random() * defaultProfileImages.length)
							]
						}`;
						await db.updateField(db.collections.DEFAULT, { _key: `user:${uid}` }, { $set: { picture: image } }, { upsert: false } );
						users[0]['picture'] = image;
					} else {
						users[0]['picture'] = usr.picture;
					}
				}
			}
