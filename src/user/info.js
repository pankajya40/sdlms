...
module.exports = function (User) {
	...
	User.getUserType = async (uid) => {
		if (!uid) throw new Error("No uid was supplied");
		const key = {
			_key: `user:${uid}`
		};
		let { userType } = await db.findField(db.collections.DEFAULT, key);
		return userType ? userType.toLowerCase() : null;
	}

	User.checkUserType = async (uid, type) => {
		if (!uid) throw new Error("No uid was supplied");
		if (!type) throw new Error("No type was supplied");
