...
module.exports = function (middleware) {
	...
	// Define a middleware function named 'thirdpartylogin'
	/**
	 * @date 05-05-2023
	 * @author mohitjakhotra
	 * @function thirtypartylogin
	 * @description middleware function for handling third-party login requests.
	 *  It checks if the requested service exists and has a "verify" function,
	 *  calls the "verify" function of the service, and if the response is truthy,
	 *  calls the "doLogin" function of the authentication controller with the request and response objects.
	 *  If there is an error,it logs it to the console.
	 *  Finally, it calls the "next" function to proceed to the next middleware or route handler.
	 */
	middleware.thirdpartylogin = helpers.try(async function thirdpartylogin(req, res, next) {
		try {
			// Extract the 'service' parameter from the request
			const service = req.params.service;

			// Check if the 'service' exists and is an object with a 'verify' function
			if (!services[service] || typeof services[service] !== 'object' || typeof services[service].verify !== 'function') {
				// If not, throw an error with a message indicating the unknown service
				throw new Error(`Unknown service: ${service}`);
			}

			// Call the 'verify' function of the service and wait for the response
			const response = await services[service].verify(req);

			// If the response is truthy, call the 'doLogin' function of the authentication controller with the request and response objects
			if (response) {
				await controllers.authentication.doLogin(req, response);
			} else {
				// Otherwise, throw an error with a generic message
