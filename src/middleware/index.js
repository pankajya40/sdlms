...
middleware.typedValidation = require("./typedValidation");
middleware.requests = require('./requests');
...
middleware.checkRequired = function (fields, req, res, next) {
	...
	const missing = fields.filter(field => !req.body.hasOwnProperty(field) || req.body[field] == '');
 ...
 * @description Checks if the user is logged in or not. If not logged in, it redirects back the user to the register page
 * @param {Object} req - Request body
 * @param {Object} res - Response
 * @param {Callback} next - Callback
 * @returns 
 */
middleware.requireRegister = function requireRegister (req, res, next) {
	if (!req.uid) {
        return res.redirect('/register?share_redirect=true&url=' + req.url);
    }
	next();
};
 ...
 * @description Tries to parse the parsable formData to JSON from the request body and returns the modified request Object
 * @param {String} fields
 * @param {*} req
 * @param {*} res
 * @param {*} next
...
middleware.parseFormDataToJSON = function parseFormDataToJSON (fields = [], req, res, next) {
	if (fields.length) {
		let body = {}
...
middleware.logRequest = async function logRequest (app, req, res, next) {
	const collectionName = db.collections.GLOBAL.REQUESTS;
	let { applications } = constants;
	let application = 'global';

	if (app) {
		if (!applications.includes(app)) {
			return controllers.helpers.formatApiResponse(400, res, new Error('Invalid Application name! Please check and use the valid Application name from the list'));
		}
		application = app;
	}

	const requestLog = {
		_key: `application:${application.toLowerCase()}:log`,
		uid: parseInt(req.uid || 0),
		application,
	};

	await db.updateField(collectionName, requestLog, {$set: {...requestLog, updatedAt: Date.now()}}, {upsert: true})
