...
function typedFieldValidation(schema) {
    ...
    return async (req, res, next) => {
        const {valid, error, result} = await schema.setCurrentRequest(req).setCurrentResponse(res).validate(Object.keys(req.body).length === 0 ? req.query : req.body)
        // req.isValid = valid
...
module.exports = {typedFieldValidation, Fields, DTID, ArrayOf, Time, File}