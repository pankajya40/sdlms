...
const {DTID, ArrayOf, Time, File} = require("./types")
...
class Fields {
    ...
    constructor(name, required = false, type="object", copyOf={}) {
         ...
         */
        this.$schema = deepCopy(copyOf.$schema != null ? copyOf.$schema : {})
        this.currentRequest = {}
    }
    ...
    add(field, options = {}) {
        ...
        const Options = ["required", ...Object.keys(Fields.validationFunctions != null ? Fields.validationFunctions : {})].reduce(
     ...
     */
    file(field, options = {}) {
        return this.add(field, {...options, type: File})
    }
    ...
    files(options = {}) {
        const Options = Object.keys(options).reduce(
            (Options, field) => ({...Options, [field]: {...options[field], type: File}})
        )
        return this.addMany(Options)
    }
    ...
    array(field, options = {}) {
        ...
        return this.add(field, {...options, type: ArrayOf(options.type != null ? options.type : "any")})
    }
    ...
    arrays(options = {}) {
        const Options = Object.keys(options).reduce(
            (Options, field) => ({...Options, [field]: {...options[field]}, type: options instanceof Fields ? options.asArray().setParent() : (ArrayOf((options[field] != null ? options[field] : {}).type != null ? (options[field] != null ? options[field] : {}).type : "any"))})
        )
        return this.addMany(Options)
    }
    ...
    async validate(object, asType = true, index = null) {
        if(this.type === "array" && asType) {
            if(!Array.isArray(object)) return {valid: false, error: `${this.name} must be an array${this.parent.name ? ` in ${this.parent.name}` : ""}!`}
        ...
        const result = deepCopy(object != null ? object : {})
        for(const key in this.$schema) {
            ...
            if(this.$schema[key] instanceof Fields) {
                ...
                const {valid, error, result: Result} = await this.$schema[key].setCurrentRequest(this.currentRequest).setCurrentResponse(this.currentResponse).validate(object[key], true, index)
                result[key] = Result
            ...
            for(const validation in this.$schema[key]) {
                const validRaw = (Fields.validationFunctions[validation] == null 
                    ...
                    ? true
                    : Fields.validationFunctions[validation](object[key], this.$schema[key][validation], key, result, this.currentRequest, this.currentResponse)
                const valid = validRaw instanceof Promise ? await validRaw : validRaw
                if(!valid) {
                    const transformedField = (Fields.validationFunctions[validation] != null ? Fields.validationFunctions[validation] : {}).transformField(this.$schema[key][validation])
                    const error = this.formatErrorMessage(key, Fields.validationFunctions[validation].error instanceof Function ? Fields.validationFunctions[validation].error(this.$schema[key][validation]) : Fields.validationFunctions[validation].error, transformedField != null ? transformedField : this.$schema[key][validation], (result[key] != null ? result[key] : {})._errorIndex != null ? (result[key] != null ? result[key] : {})._errorIndex : index)
                    return {
                        valid,
                        error,
                        result
 ...
 * @param {(actual: ObjT, value: T, key: string, parent: string, req?: object, res?: object) => boolean} ValidationCallback - callback to validate against given name
