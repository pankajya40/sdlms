...
async function registerAndLoginUser(req, res, userData) {
	userData.personal_assets_id = await db.incrObjectField('global', 'nextTid');
...
authenticationController.register = async function (req, res) {
	...
	if(!userData.role || userData.role.length == 0) userData.role = "guest" 
	try {
...
function continueLogin(req, res, next) {
	passport.authenticate('local', async function (err, userData, info) {
		...
		if (userData.passwordExpiry && userData.passwordExpiry < Date.now()) {
			...
			await authenticationController.doLogin(req, userData.uid, userData);
			var destination;
			if (req.session.returnTo) {
...
authenticationController.doLogin = async function (req, uid, userData = {}) {
	if (!uid) {
		return;
	}
	const loginAsync = util.promisify(req.login).bind(req);
	await loginAsync({ uid: uid });
	await authenticationController.onSuccessfulLogin(req, uid, userData);
};

authenticationController.onSuccessfulLogin = async function (req, uid, userData = {}) {
	/*
	...
	try {
		...
		req.session.meta.personal_assets_id = userData.personal_assets_id || 0;
