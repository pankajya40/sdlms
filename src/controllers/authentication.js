...
const { stringify } = require('../../public/src/modules/helpers');
	...
	plugins.hooks.fire('filter:register.interstitial', {
		...
		var done = function (err, data) {
			...
			if (req.session.returnTo) {
				...
				*/
				try {
					let redirect_url  = res.req.cookies.share_redirect;
					/**
					 * @author imshawan [12-04-2022]
					 * @description The redirection was causing issue after registeration is complete if 'redirect_url' doesn't exist
					 * therefore added a check for that
					 */
					if (redirect_url) {
						res.redirect(nconf.get('relative_path') + redirect_url);
					} else { 
...
authenticationController.login = function (req, res, next) {
	...
	plugins.hooks.fire('filter:login.check', {
		req: req,
		res: res,
		userData: req.body
	}, (err) => {
	...
	passport.authenticate('local', async function (err, userData, info) {
		...
		if (userData.passwordExpiry && userData.passwordExpiry < Date.now()) {
			...
			if (req.body.noscript === 'true') {
				 ...
				 * @author imshawan
				 * @date 07-01-2023
				 * @description Returning session information so that it can be saved in mobile app for the
				 * retrival of session data
				 */
				res.status(200).send({
					next: destination,
					session: req.session,
...
authenticationController.logout = async function (req, res, next) {
	...
	try {
		...
		await clearUserRequestLogs(uid);
...
async function clearUserRequestLogs (uid) {
	await db.removeField(db.collections.GLOBAL.REQUESTS, {uid})
}
