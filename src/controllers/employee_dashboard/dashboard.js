...
dashboardController.get = async function (req, res, next) {
	...
	var [userData, userGroupData, isTeacher, isStudent, Sessions, 
		...
		user.getUsersFields([uid], fields),
		groups.getUserGroups([uid]),
		privileges.users.isTeacher(uid),
	...
	parentDashboardData.current_month = current_month;
	parentDashboardData.hideExtendedNav=true;
	res.render("sdlms/employee_dashboard/dashboard", parentDashboardData);
};
