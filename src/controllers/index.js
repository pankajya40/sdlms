...
Controllers.employee_dashboard = require('./employee_dashboard');
...
Controllers.toc = require('./toc');
Controllers.scorecard = require('./scorecard');
Controllers.socialquiz = require('./socialquiz');
Controllers.posterGenerator = require('./postergenerator');
Controllers.profile = require('./profile');
Controllers.sdlmsAdmin = require('./admin/sdlmsAdmin');
Controllers.payments_page = require('./payments_pages');
// controller for profile page
Controllers.resumeProfile = require('./resumeProfile');
Controllers.contentManager = require('./contentManager');
//Controllers.workshopmanager = require('./workshopmanager')
Controllers.workshop = require('./workshop')
Controllers.communitybuilder = require('./communitybuilder')
Controllers.contentManager=require('./contentManager');

Controllers.applicationManager = require('./applicationManager')

// Controllers.journal = require("./journal");
Controllers.communication = require("./communication");
Controllers.cccms = require('./cccms');

/**
 * @author imshawan (07-09-2022)
 * @description Added controller for PDGMS, avoid using any other controllers for PDGMS.
 */
Controllers.PDGMS = require('./PDGMS');
Controllers.DTForms = require('./DTForms');
Controllers.observation = require('./observation');
Controllers.dashboard = require('./dashboard');
Controllers.dataportal = require('./dataportal');
Controllers.generators = require('./generators');
Controllers.rigorBuilder = require('./rigorbuilder');
...
Controllers.login = async function (req, res) {
	...
	let googleQuery = new URLSearchParams(params).toString();
	const googleLoginUrl = `https://accounts.google.com/o/oauth2/v2/auth?${googleQuery}`;
...
Controllers.manifest = async function (req, res) {
	...
	manifest.icons.push(
		{
			src: 'https://sdlms.deepthought.education/assets/uploads/files/brand_icons/ic_launcher_sdpi.png',
			sizes: '36x36',
			type: 'image/png',
			density: 0.75,
		},
		{
			src: 'https://sdlms.deepthought.education/assets/uploads/files/brand_icons/ic_launcher_mdpi.png',
			sizes: '48x48',
			type: 'image/png',
			density: 1.0,
		},
		{
			src: 'https://sdlms.deepthought.education/assets/uploads/files/brand_icons/ic_launcher_hdpi.png',
			sizes: '72x72',
			type: 'image/png',
			density: 1.5,
		},
		{
			src: 'https://sdlms.deepthought.education/assets/uploads/files/brand_icons/ic_launcher_xhdpi.png',
			sizes: '96x96',
			type: 'image/png',
			density: 2.0,
		},
		{
			src: 'https://sdlms.deepthought.education/assets/uploads/files/brand_icons/ic_launcher_xxhdpi.png',
			sizes: '144x144',
			type: 'image/png',
			density: 3.0,
		},
		{
			src: 'https://sdlms.deepthought.education/assets/uploads/files/brand_icons/ic_launcher_xxxhdpi.png',
			sizes: '192x192',
			type: 'image/png',
			density: 4.0,
		},
		{
			src: 'https://sdlms.deepthought.education/assets/uploads/files/brand_icons/ic_launcher_xxxxhdpi.png',
			sizes: '512x512',
			type: 'image/png',
			density: 10.0,
		}
...
Controllers.serviceWorker = async function (req, res) {
	// read the service worker file
	let serviceWorker = await fs.readFile((__dirname + '/../../public/src/service-worker.js'), 'utf8');
	serviceWorker = (`const cacheBuster = '${(meta.config['cache-buster']).split('=')[1]}'`) + serviceWorker;
	// send as javascript
	res.set('Content-Type', 'application/javascript');
	res.status(200).send(serviceWorker);
};

Controllers.questionBuilder = async function (req, res) {

	let html = await fs.readFile((__dirname + '/../../public/src/html/question-builder.html'), 'utf8');
