...
eagleBuildersController.get = async function (req, res, next) {
	...
	const pagination = {
		isPrev: page > 0,
		first: `/myassets/eaglebuilders?${qs}`,
		prev: `/myassets/eaglebuilders/${page}?${qs}`,
		current: page + 1,
		total: (Math.ceil(total / perPage) || 1),
		next: `/myassets/eaglebuilders/${page + 2}?${qs}`,
		last: `/myassets/eaglebuilders/${(Math.ceil(total / perPage) || 1)}?${qs}`,
