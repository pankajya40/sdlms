...
threadbuildersController.get = async function (req, res, next) {
	...
	const pagination = {
		isPrev: page > 0,
		first: `/myassets/threadbuilders?${qs}`,
		prev: `/myassets/threadbuilders/${page}?${qs}`,
		current: page + 1,
		total: (Math.ceil(total / perPage) || 1),
		next: `/myassets/threadbuilders/${page + 2}?${qs}`,
		last: `/myassets/threadbuilders/${(Math.ceil(total / perPage) || 1)}?${qs}`,
	...
	res.render('sdlms/assets/threadbuilders/index', {
		...
		hideSidebar: req.query.no_sidebar ? true : false,
