...
profileController.get = async function (req, res, next) {
	...
	userData.isStudent = await privileges.users.isStudent(req.uid);
...
async function getPosts(callerUid, userData, setSuffix) {
	...
	const keys = cids.map(
		(c) => "cid:" + c + ":uid:" + userData.uid + ":" + setSuffix
	);
	...
	var retPost = postData.slice(0, count);

	var newRetPost = retPost.map((it) => {
		let min = (it.content || '').replace(/(<([^>]+)>)/g, "");
		it.miniContent = min.substring(0, 110) + "...";
		return it;
	});
...
function addMetaTags(res, userData) {
	var plainAboutMe = userData.aboutme
