...
connection.getConnectionString = function (mongo) {
	...
	var sdlms_env = nconf.get('sdlms_env');

	var usernamePassword = '';
	var uri = mongo.uri || '';
	if (sdlms_env && sdlms_env.toLowerCase() == 'development'){
		let dev_uri = mongo.dev_uri;
		if (dev_uri) {
			uri = dev_uri;
			winston.warn('SDLMS is using development database!');
		}
	} else winston.warn('SDLMS is using production database!');
