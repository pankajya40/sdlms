...
module.exports = function (module) {
	...
	module.decrObjectFieldCount = async function (collection, key, field) {
		if (!await module.client.collection(collection).findOne(key, { [field]: { $exists: false, $ne: null } })) {
			await module.client.collection(collection).update(key, { $set: { [field]: 0 } });
		}
		await module.client.collection(collection).findOneAndUpdate(key, { $inc: { [field]: -1 } }, { returnOriginal: false, upsert: true });
	};
	...
	module.findFields = async (collection, keys, fields = [], order={$natural: -1}) => {
		validateCollection(collection);
		if (fields.length) {
			const records = await module.client.collection(collection).find(keys).sort(order).toArray();
