...
const initCollection = require('./collector');
...
exports.listen = async function () {
	...
	// await initializeDataCollection(app); // Disabled as of now
...
function listen(callback) {
	...
	//var bind_address = ((nconf.get('bind_address') === '0.0.0.0' || !nconf.get('bind_address')) ? '127.0.0.1' : nconf.get('bind_address'));
...
async function initializeDataCollection(expressApp) {
	initCollection();
	winston.info('Data Collector Initialized');
}
