...
appApi.getDiscussionRoom = async req => {
	...
	if (req.query.tid) {
		let roomId = parseInt(req.query.tid);
		let key = { roomId, type: 'discuss_room' }
...
appApi.createDiscussionRoom = async (req) => {
	...
	const Data = {
		...
		min_rigor_rank: req.body.min_rigor_rank,
	}
...
appApi.getEvents = async (data) => {
	...
	const event_type = data.query.type || 'latest';
	const page = parseInt(data.query.page) || 0;
	...
	console.log("Huhuehueh", `/app${data.url}`);
...
appApi.createEvent = async (data) => {
	const luid = parseInt(data.uid);
	if (!data.uid || luid < 1) throw new Error("Unauthorized");
	console.log("create event", data.body);
	console.log("aparna", data.body);
	const collectionName = db.collections.DEFAULT;
	let key = {
		type: "event",
		name: data.body.name,
		description: data.body.description,
		description_1: data.body.description_1,
		description_2: data.body.description_2
	}
	let post = await db.findField(collectionName, key);
	console.log(post, "dfbdsnmf");
	if (post !== null) {
		return { msg: "this event has been already created!!!" }
	}
...
appApi.create_Post = async (data) => {
	...
	if (data.body.isDraft) {
		Post.status = JSON.parse(data.body.isDraft.toLowerCase()) ? 'draft' : 'published';
	}
	...
	let post = await createPost(Post);
	return { ...post, pid };
};
...
appApi.getPosts = async (data) => {
	...
	const page = parseInt(data.query.page) || 0;
	const limit = parseInt(data.query.limitBy) || 10;
	const post_type = data.query.type || "latest";
	const order = {};
	const MAX_CHARS_PER_POST = 210;
	...
	if (data.query.pid) {
		...
		let key = { pid: pid, type: 'post', 'uuid': { $exists: false } }
		let post = await db.findField(collectionName, key)
		let attachment;

		if (post.attachment_id && post.attachment_id != 'NaN') {
			let query = {};
			if (isNaN(post.attachment_id)) {
				query._id = ObjectId(post.attachment_id);
			} else {
				query.pid = parseInt(post.attachment_id);
			}
		...
		let [userData, _] = await Promise.all([
			...
			// db.countDocuments(collectionName, { pid: pid, type: 'comment' }),
		...
		let post_data = { ...post, viewcount: post.viewcount + 1, user: userData, attachment: attachment }
		...
		preferences.forEach(preference => {
			...
			if (preference.hasOwnProperty('sub_cids')) {
				preference['sub_cids'].forEach(sub_cid => {
					let key = { 'uuid': { $exists: false }, content: { $exists: true }, status: { $ne: 'draft' } }
					key['sub_cid'] = sub_cid;
					key['cid'] = preference['cid'];
					key['type'] = 'post'
					filter.push(key)
		...
		const term = data.query.term;
		[postData, count] = await Promise.all([
			db.getFieldsWithPagination(collectionName, { content: { $regex: term, $options: '$i' }, type: 'post', status: { $ne: 'draft' } }, limit, page),
			db.countDocuments(collectionName, { content: { $regex: term, $options: '$i' }, type: 'post', status: { $ne: 'draft' } })
		]);
	...
	let posts = await Promise.all(postData.map(async (elem) => {
		...
		let [userData] = await Promise.all([
			User.getUserFields([elem.uid], userFields),
			//db.countDocuments(collectionName, { pid: elem.pid, type: 'comment' })
		])
		if (elem.attachment_id && elem.attachment_id != 'NaN') {
			let query = {};
			if (isNaN(elem.attachment_id)) {
				query._id = ObjectId(elem.attachment_id);
			} else {
				query.pid = parseInt(elem.attachment_id);
			}
			attachment = await db.findField(collectionName, query);
		}
		let content;
...
appApi.updatePost = async (data) => {
	...
	if (data.body.cid && data.body.cid != 'NaN') parsedData.cid = data.body.cid;
	if (data.body.sub_cid && data.body.sub_cid != 'NaN') parsedData.sub_cid = data.body.sub_cid;
	if (data.body.isDraft) {
...
function loopOver(arr, parent_categories, sub_categories) {
	let sub_cats = [];
	let p = {
	}
	const errors = [];
	arr.forEach((preference) => {
		...
		console.log(errors, p, "kdslfkdls");
	});
	if (errors.length > 0) {
		console.log(errors, "error");
		return ({ error: true, sucess: false, data: errors });
	}
	else {
		p = { ...p, sub_cats };
		console.log(p, "opop");
		return { error: false, sucess: true, data: p };
	}
}
appApi.savePreferences = async (data) => {
	console.log(data.uid, "chini");
	const luid = parseInt(data.uid);
	if (!data.uid || luid < 1) throw new Error("Unauthorized");
	const collectionName = db.collections.DEFAULT;
	...
	const categoryPreferences = data.body.preferences;
	console.log(categoryPreferences, "nbnb");
	const preferences = {
		global: [],
		feed: [],
		events: [],
	...
	const isempty = JSON.stringify(categoryPreferences) === '{}';
	...
	if (!isempty) {
		...
		console.log(categoryPreferences, "e");
		const sub_categories = categoryData.filter((e) => e.parentCid > 0);
		const parent_categories = categoryData.filter(
			(e) => e.parentCid < 1 || e.parentCid === undefined
		);
		...
		//for global
		if (categoryPreferences.global.length > 0) {
			var globalObj = loopOver(categoryPreferences.global, parent_categories, sub_categories);
			console.log(globalObj);
			if (globalObj && globalObj.error) {
				errors.push(globalObj.data);
			}
			else if (globalObj) {
				preferences.global.push(globalObj.data);
			}
		}
		if (categoryPreferences.feed.length > 0) {
			const feedObj = loopOver(categoryPreferences.feed, parent_categories, sub_categories);
			console.log(feedObj);
			if (feedObj && feedObj.error) {
				errors.push(feedObj.data);
			}
			else if (feedObj) {
				preferences.feed.push(feedObj.data);
			}
		}
		if (categoryPreferences.events.length > 0) {
			const eventsObj = loopOver(categoryPreferences.events, parent_categories, sub_categories);
			console.log(eventsObj);
			if (eventsObj && eventsObj.error) {
				errors.push(eventsObj.data);
			}
			else if (eventsObj) {
				preferences.events.push(eventsObj.data);
			}
		}
		if (categoryPreferences.nudge.length > 0) {
			const nudgeObj = loopOver(categoryPreferences.nudge, parent_categories, sub_categories);
			console.log(nudgeObj);
			if (nudgeObj && nudgeObj.error) {
				errors.push(nudgeObj.data);
			}
			else if (nudgeObj) {
				preferences.nudge.push(nudgeObj.data);
			}
		}
		if (categoryPreferences.discussion.length > 0) {
			const discussionObj = loopOver(categoryPreferences.discussion, parent_categories, sub_categories);
			console.log(discussionObj);
			if (discussionObj && discussionObj.error) {
				errors.push(discussionObj.data);
			}
			else if (discussionObj) {
				preferences.discussion.push(discussionObj.data);
			}
		}
		console.log(preferences, "dsmndm");
		if (errors.length) throw new Error(JSON.stringify({ errors }));
		else await db.update(collectionName, key, { $set: { preferences: preferences } })
	}
	else {
...
appApi.createArticle = async (data) => {
	...
	const resp = await db.setField(db.collections.DEFAULT, article);
	return { pid, ...resp };
};
...
async function createPost(data) {
	...
	return { tid: topicData.tid, ...topicData };
}
 ...
 * @author Arbaaz
 * @date 11th June
 * @functions to create update delete and get the reflections templates
 */

appApi.createReflectionTemplate = async (req) => {
	const uid = parseInt(req.uid);

	const collectionName = db.collections.MOBILE.REFLECTION_TEMPLATE;
	const cid = await getCustomCategoryId("reflection_template");
	const tid = await db.incrObjectField("global", "nextTid");
	const currentTime = Date.now();

	const { description, title, blocks } = req.body;
	let parsedBlock;
	if (blocks) {
		try {
			parsedBlock = JSON.parse(blocks);
		} catch (err) {
	...
	const reflectionTemplate = await db.setField(collectionName, Data);
	if (!reflectionTemplate) throw new Error("Could not create reflection template");
	return reflectionTemplate;
}

appApi.getReflectionTemplates = async (req) => {
	const collectionName = db.collections.MOBILE.REFLECTION_TEMPLATE;
	...
	let count = 0;
	let keys = { type: "reflection_template" };

	if (req.query.tid) {
		reflectionTemplates = await db.findField(collectionName, { tid: parseInt(req.query.tid), type: 'reflection_template' });
		count = 1;
	} else {
		[reflectionTemplates, count] = await Promise.all([
			db.getFieldsWithPagination(collectionName, keys, limit, page),
			db.countDocuments(collectionName, keys),
		]);
	}

	return utilities.paginate(`/app${req.url}`, reflectionTemplates, count, limit, page);
}


appApi.updateReflectionTemplate = async (req) => {
	const uid = parseInt(req.uid);
	const collectionName = db.collections.MOBILE.REFLECTION_TEMPLATE;
	...
	if (req.body.blocks) parsedData.blocks = req.body.blocks;
...
appApi.deleteReflectionTemplate = async (req) => {
	const uid = parseInt(req.uid);
	const keys = {
		uid,
		type: "reflection_template",
...
appApi.createReflection = async (req) => {
	const uid = parseInt(req.uid);
	...
	const { summary, blocks, tid, parent_pid, parent_type } = req.body;
	...
	let Data = {
		...
		summary,
		blocks,
		parent_pid: parseInt(parent_pid),
		parent_type,
		createdAt: currentTime,
		updatedAt: currentTime,
		type: "reflection",
	...
	const [reflection] = await Promise.all([
		db.setField(collectionName, Data),
		db.incrementCount(db.collections.DEFAULT, { pid: parseInt(parent_pid) }, 'reflection_count')
	]);
	if (!reflection) throw new Error("Could not create reflection");
	return reflection;
}

appApi.getReflection = async (req) => {
	const collectionName = db.collections.MOBILE.REFLECTION;
	...
	let reflection;
	let count = 0;
	let keys = { type: "reflection" };

	if (req.query.pid) {
		reflection = await db.findFields(collectionName, { pid: parseInt(req.query.pid), ...keys });
		count = 1;
	} else if (req.query.parent_pid) {
		reflection = await db.findFields(collectionName, { parent_pid: parseInt(req.query.parent_pid), ...keys });
		[reflection, count] = await Promise.all([
			db.getFieldsWithPagination(collectionName, { parent_pid: parseInt(req.query.parent_pid), ...keys }, limit, page),
			db.countDocuments(collectionName, { parent_pid: parseInt(req.query.parent_pid), ...keys }),
		]);
	} else if (req.query.uid) {
		let uid = parseInt(req.query.uid);
		[reflection, count] = await Promise.all([
			db.getFieldsWithPagination(collectionName, { ...keys, uid }, limit, page),
			db.countDocuments(collectionName, { ...keys, uid }),
		]);
	} else {
		[reflection, count] = await Promise.all([
			db.getFieldsWithPagination(collectionName, keys, limit, page),
	...
	const reflectionData = await Promise.all(reflection.map(async (item) => {
		let userData = await User.getUserFields([item.uid], ["username", "fullname", "userslug", "picture"]);
		return { ...item, author: userData };
	}))
...
appApi.updateReflection = async (req) => {
	const uid = parseInt(req.uid);
	const collectionName = db.collections.MOBILE.REFLECTION;
	...
	let parsedData = {};
	if (req.body.title) parsedData.title = req.body.title;
	if (req.body.summary) parsedData.summary = req.body.summary;
	if (req.body.blocks) parsedData.blocks = req.body.blocks;
...
appApi.deleteReflection = async (req) => {
	const uid = parseInt(req.uid);
	const keys = {
		uid,
		type: "reflection",
...
const { app } = require("../controllers/write");
 ...
 * @description Created a comment based on a parent asset/object
 * @param {Object} data
 * @returns
 */

appApi.createComment = async function (data) {
	const uid = parseInt(data.uid);
	const collectionName = db.collections.GLOBAL.COMMENT;

	const asset_owner_uid = parseInt(data.body.asset_owner_uid);
	// const [asset_owner] = await Promise.all([
	// 	User.getUserFields([uid], ['picture', 'fullname']),
	// 	User.getUserFields([asset_owner_uid], ['fullname']),
	// ]);
	const currentTime = Date.now();
	const pid = await db.incrObjectField("global", "nextPid");
	const { attachment_id } = data.body;
	//const commentTo = await User.getUserFields([commentFor.userId], ['picture', 'fullname']);
	...
	const comment = {
		...
		comment_for: data.body.attachment_type,

		// We will have to get the user info while fetching the comments
	...
	if (data.body.parent) {
		comment.parent = data.body.parent;
	}

	let count_key;
	if (isNaN(attachment_id) && attachment_id != 'NaN') {
		count_key = { _id: ObjectId(attachment_id) };
	} else {
		count_key = { pid: attachment_id };
	}

	if (count_key) {
		await db.incrementCount(db.collections.DEFAULT, count_key, 'comment_count');
	}

	const resp = await db.setField(collectionName, comment);
	return {
		id: resp._id,
	};
};

appApi.updateComment = async function (data) {
	const uid = parseInt(data.uid);
	const collectionName = db.collections.GLOBAL.COMMENT;
	...
	if (data.body.content) {
		parsedData.content = data.body.content;
	}
	parsedData.modified = currentTime;
...
appApi.deleteComment = async function (data) {
	const uid = parseInt(data.uid);
	const collectionName = db.collections.GLOBAL.COMMENT;
	const keys = {
		creator: uid,
	...
	let comment = await db.findField(collectionName, keys);
	if (comment) {
		let actualPost = {
			_id: ObjectId(comment.attachment_id)
		}
		const [state, _] = await Promise.all([
			db.removeField(collectionName, keys),
			db.decrObjectFieldCount(db.collections.DEFAULT, actualPost, 'comment_count')
		])
		return {
			deleted: state.result.n === 1,
		};
	} else throw new Error('Invalid commend Id! No comment was found');
};

appApi.postGlobalNotifications = async function (req) {
	const collectionName = db.collections.DEFAULT;
	const tid = await db.incrObjectField("global", "nextTid");
	// const keys = { type : 'globalNotification' };
	const type = 'globalNotification'
	...
	try {
		...
		const payload = { assetType, assetId, title, content, type, active, tid };
...
appApi.getGlobalNotifications = async function (req) {
	const collectionName = db.collections.DEFAULT;
	const keys = { type: "globalNotification", active: true };
	const data = await db.findFields(collectionName, { ...keys });
	if (!data) throw new Error("Cannot get data");
	return data;
}

appApi.deleteGlobalNotifications = async function (req) {
	const collectionName = db.collections.DEFAULT;
	const tid = req.body.tid;
	console.log(tid);
	const keys = { type: "globalNotification", active: true, tid: tid };
	const payload = { active: false };

	const result = await db.update(collectionName, keys, { $set: payload });
	// const result = await db.updateFieldWithMultipleKeys(collectionName, keys , payload );

	console.log(result);
	return result;
}

appApi.getMyDiscussions = async (req) => {
	const uid = parseInt(req.uid);
	const rooms = await Messaging.getRoomIds();
	const result = await Messaging.isMemberOfRoom(uid, rooms);
	return result;
}

appApi.getMyMess = async (req) => {
	const result = await Messaging.loadRoom(req.uid, req.body);
	console.log(result);
	console.log(result.messages.length);
	return result;
}

appApi.getComments = async function (data) {
	const collectionName = db.collections.GLOBAL.COMMENT;
	const userFields = ['picture', 'fullname', 'username'];
	if (data.query &&
	...
	//----
	if (data.query.id) {
		let commentData = await db.findFields(collectionName, {
			...keys,
			attachment_id: data.query.id,
		});
		if (!commentData.length) return [];

		return await Promise.all(commentData.map(async (comment) => {
			const [userData, creator] = await Promise.all([
				User.getUserFields([comment.asset_owner_uid], userFields),
				User.getUserFields([comment.creator], userFields),
			])
			return {
				...comment,
				asset_owner_name: userData.fullname || userData.displayname || userData.username,
				fullname: creator.fullname || creator.displayname || creator.username,
				profile_picture_url: creator.picture,
				comment_to: userData,
			};
		}));
	} else return [];
 ...
 * @date 29-06-2022
 * @function getFeeds
 * @description The main method of the mobile app home screen that is responsible for collecting the
 * assets from the database, generate a paginated response and serve the client
 * @param {Object} data
 * @returns Paginated array of feeds
 */

appApi.getFeeds = async (data) => {
	const collectionName = db.collections.DEFAULT;
	const userFields = [
		"username",
		"fullname",
	...
	const searchIn = ['post', 'article'];

	const page = parseInt(data.query.page) || 0;
	const limit = parseInt(data.query.limitBy) || 5;
	const feed_type = data.query.type || "latest";
	const order = {};
	...
	if (data.query.pid) {
		let pid = parseInt(data.query.pid);
		let key = { pid: pid, 'uuid': { $exists: false } }
		let feed = await db.findField(collectionName, key)
		let attachment;

		if (feed.attachment_id && feed.attachment_id != 'NaN') {
			let query = {};
			if (isNaN(feed.attachment_id)) {
				query._id = ObjectId(feed.attachment_id);
			} else {
				query.pid = parseInt(feed.attachment_id);
			}
			attachment = await db.findField(collectionName, query);
		}

		let [userData, _] = await Promise.all([
			await User.getUserFields([feed.uid], userFields),
			db.incrementCount(collectionName, key, 'viewcount') // Increment view count for a particular item
		]);

		// Incrementing the view count here itself, because the view count gets increased only after that record is called
		// To avoid an additional db query its best to increment the count here manually by 1
		let feed_data = { ...feed, viewcount: feed.viewcount + 1, user: userData, attachment: attachment }
		return utilities.paginate(`/app${data.url}`, feed_data, count, limit, page)
		...
		[feedsData, count] = await Promise.all([
			db.getFieldsWithPagination(collectionName, search, limit, page, order),
			db.countDocuments(collectionName, search)
		]);
	} else if (data.query.term) {

		const term = new RegExp(data.query.term);
		[feedsData, count] = await Promise.all([
			db.getFieldsWithPagination(
				collectionName,
				{
	...
	const feeds = await Promise.all(feedsData.map(async (elem) => {
		let attachment = null;
		let userData = await User.getUserFields([elem.uid], userFields);
		if (elem.attachment_id && elem.attachment_id != 'NaN') {
			let query = {};
			if (isNaN(elem.attachment_id)) {
				query._id = ObjectId(elem.attachment_id);
	...
	return utilities.paginate(`/app${data.url}`, feeds, count, limit, page);
};

appApi.getDrafts = async (data) => {
	const collectionName = db.collections.DEFAULT;
	const uid = parseInt(data.uid);
	...
	if (type) {
		if (searchIn.includes(type)) {
			[assetsData, count] = await Promise.all([
				db.getFieldsWithPagination(collectionName, { ...keys, type }, limit, page),
				db.countDocuments(collectionName, { ...keys, type }),
			]);
	...
	const draftItems = await Promise.all(assetsData.map(async (elem) => {
		let attachment = null;
		let userData = await User.getUserFields([elem.uid], userFields);
		if (elem.attachment_id && elem.attachment_id != 'NaN') {
			let query = {};
	...
	return utilities.paginate(`/app${data.url}`, draftItems, count, limit, page);
}
