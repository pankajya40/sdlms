...
class Stepper {
	...
	init() {
		this.$target.find('[sdlms-step]').hide().removeClass('active');
		this.$target.find('[sdlms-step]').eq(this.active).show().addClass('active');
	}
	next() {
		let $elem = this.$target.find('[sdlms-step].active');
		let index = ($elem.index() + 1);
		let $next = this.$target.find('[sdlms-step]').eq(index);

		if (!this.loop && !$next.length) return console.log('No Next Step')
		else if (this.loop && !$next.length) $next = this.$target.find('[sdlms-step]').eq(0);

		this.$target.find('[sdlms-step]').hide().removeClass('active');
		$next.addClass('active').show();

		this.$target.trigger('sdlms:step:change', {
	...
	prev() {
		let $elem = this.$target.find('[sdlms-step].active');
		let index = ($elem.index() - 1);
		let $prev = this.$target.find('[sdlms-step]').eq(index);

		if (!this.loop && !$prev.length) return console.log('No Next Step')
		else if (this.loop && !$prev.length) $prev = this.$target.find('[sdlms-step]').eq(this.$target.find('[sdlms-step]').length - 1);
	...
	go(index) {
		let $prev = this.$target.find('[sdlms-step].active');
		let $elems = this.$target.find('[sdlms-step]');
		let $elem = $elems.eq(index - 1);
		if (!$elem.length) return console.log('No Step Found');
		$elems.hide().removeClass('active');
...
((()=>{
	// change this to get parammeter by name function 
	location.href.includes("header=false") && $('header').addClass('d-none')
})());