...
define('forum/mobile/message/chat', function () {
	...
	function groupByDay(scroll = true) {
		...
		var getDateLabel = (timestamp) => {
			...
			return moment(timestampDay).format('MMM DD, YYYY');
		};
	...
	function processMessage(message, old) {
		...
		try {
			...
			if (template != 'message') message.content = message.cleanedContent;
	...
	chat.init = function () {
		...
		const sender = ajaxify.data.users[0] || {}; // Needs to be taken care of
		$('#profile-info').html(`
	...
	chat.events = function () {
		$(window).on('scroll', function () {
			...
			if (!chat.loadingChats) {
				...
				const scPercent = (topScroll / scrollableHeight) * 100;
				if (scPercent < 5) {
