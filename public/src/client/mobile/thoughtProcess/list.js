...
define('forum/mobile/thoughtProcess/list', function () {
	var list = {};
	...
	list.init = function () {
		...
		doAjax({
			...
			$.map(templates, template => $('#list-container').append(_templates.templateTile(template)));
        ...
        $("#list-container").on("click", ".template-tile", function () {
            ...
            ajaxify.data.parent ? ajaxify.go(`/mobile/tp/form/${$(this).attr("tid")}?ptype=${ajaxify.data.ptype}&parent=${ajaxify.data.parent}`) : ajaxify.go(`/mobile/tp/form/${$(this).attr("tid")}?ptype=${ajaxify.data.ptype}`);
        })
    };
