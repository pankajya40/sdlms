  ...
  function (api) {
    ...
    toc.events = () => {
      ...
      $("#next-week").on("click",function(e){
        e.preventDefault();
        console.log(ajaxify.data.currentDate)
        ajaxify.go("/toc/garuda/"+toc.getNextMonday(toc.currentDate));
      })

      $("#prev-week").on("click",function(e){
        e.preventDefault();
        ajaxify.go("/toc/garuda/"+toc.getPreviousWeekMonday(toc.currentDate));
      })
    ...
    toc.dirty = () => {
      console.log(ajaxify.data.weekNum)
      if (!toc.editable) return;
      toc.isDirty = true;
      let data = $('#toc').serializeObject();
      let payload = {
        pid: toc.pid,
        weekNum: ajaxify.data.weekNum,
        ...
        single: function (data, ind) {
          ...
          return data.map((item, index) => {
            return (`<tr>` +
                   ...
                   (!index ? `<td rowspan="${data.length}"><textarea ${toc.editable ? '' : 'disabled'} name="slots[${ind}][${index}][Monday]" class="input">${toc.getValueByIndex(ind, index, 'Monday')}</textarea></td>
                   <td rowspan="${data.length}"><textarea ${toc.editable ? '' : 'disabled'} name="slots[${ind}][${index}][Tuesday]" class="input">${toc.getValueByIndex(ind, index, 'Tuesday')}</textarea></td>
                   <td rowspan="${data.length}"><textarea ${toc.editable ? '' : 'disabled'} name="slots[${ind}][${index}][Wednesday]" class="input">${toc.getValueByIndex(ind, index, 'Wednesday')}</textarea></td>
                   <td rowspan="${data.length}"><textarea ${toc.editable ? '' : 'disabled'} name="slots[${ind}][${index}][Thursday]" class="input">${toc.getValueByIndex(ind, index, 'Thursday')}</textarea></td>
                   <td rowspan="${data.length}"><textarea ${toc.editable ? '' : 'disabled'} name="slots[${ind}][${index}][Friday]" class="input">${toc.getValueByIndex(ind, index, 'Friday')}</textarea></td>
                   <td rowspan="${data.length}"><textarea ${toc.editable ? '' : 'disabled'} name="slots[${ind}][${index}][Saturday]" class="input">${toc.getValueByIndex(ind, index, 'Saturday')}</textarea></td>` : '')
                    + `</tr>`)
          }).join('');
        }
      }
