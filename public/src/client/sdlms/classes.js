	...
	Sessions.init = async function () {
		...
		$modal.on('click', '#date-picker', function () {
			...
			if (input.attr('type') == 'text') {
				...
				$(this).html('<i class="fas fa fa-times"></i>');
				input.prop('readonly', false)
			} else {
				input.attr('type', 'text');
				input.val('Instant')
				$(this).html('<i class="fas fa fa-calendar-alt"></i>');
	...
	Sessions.template = function (part, data) {
		let components = {
			session: `<tr data-id="${data.tid}">
                ...
                <td > <a href="javascript:void(0)"><i class="fas fa fa-link mr-1" data-toggle="modal" data-target="#generateLink" sharer-link="${data.tid}"></i></a> <a class='button edit' title='Edit' ><i class='fa fa-pencil'></i></a> 
