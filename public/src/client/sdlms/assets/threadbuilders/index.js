	...
	THREADBUILDERS.init = () => {
		...
		$('[show-thread-builder]').on('click', function () {
			...
			new threadBuilder({
				...
				with: data,
				showHeader: app.user.personal_assets_id == (tb.tid || tb.topicId),
				noDraft: true,
				noTracking: true,
				addFeedbacks: !true
			});
		...
		$('[delete-asset]').on('click', function () {
			let id = $(this).attr('delete-asset');
			let tb = ajaxify.data.data.find(tb => tb.pid == id);
			if (tb.tid != app.user.personal_assets_id) return notify('You can not delete this threadbuilder', 'error');
			if (confirm('Are you sure you want to delete this thread builder?')) {
				api.del('/sdlms/asset', {
					pid: id,
					type: 'threadbuilder',
					uid: app.user.uid,
					...
					notify('Thread builder deleted', 'success');
					ajaxify.refresh();
				}).catch((error) => {
					if (error.message) notify(error.message, 'error');
				})
							...
							<div class="image-box position-relative">
								...
								<div class="position-absolute w-100 floating-bottom" style="${bg == 'white' ? 'background:black;color:white' : ''}">
									<div class="d-flex justify-content-between">
										<span class="text-ellipse text-left w-80 pr-2">${(data.tid || data.topicId) == app.user.personal_assets_id ? ((data.meta || {}).title || 'Personal Threadbuilder') : ''}${data.topic && data.topic.topic ? data.topic.topic : ''}</span>
										<span class="nowrap"><a href="/myassets/threadbuilders/manage/${data.pid || data._id}"><i class="fas fa-expand mr-2"></i></a> <i class="fas mr-2 fa-share-alt cursor-pointer ${data.pid ? '' : 'd-none'}" get-share-link="${data.sharer ? data.sharer.link : false}" data-expiry="${data.sharer ? data.sharer.expireAt : 0}"  data-pid="${data.pid}"></i><i class="fas cursor-pointer ml-1 ${(data.tid || data.topicId) == app.user.personal_assets_id ? '' : "d-none"} fa-trash " delete-asset="${data.pid}"></i></span>
									</div>
								</div>
							</div>
						</div>
