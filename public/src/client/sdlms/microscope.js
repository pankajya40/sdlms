...
define('forum/sdlms/microscope', ['api', 'https://cdnjs.cloudflare.com/ajax/libs/Chart.js/3.7.1/chart.min.js', "sdlms/eaglebuilder", "sdlms/threadbuilder", "sdlms/feedbacks"], function (api) {
	...
	Microscope.assignedcolor = ['#ff957f', '#ffff8a', '#bb8aff', '#bfffa5', '#ffa5b5']
	...
	Microscope.initMembers = () => {
		...
		api.get(`/sdlms/${Microscope.tid}/attendance`, {
			...
			.then((r) => {
				$('[data-target="search-student"]').each(function (index,e) {
					let i = 't' + index++;
	...
	Microscope.initSearch = () => {
		// Allowing user to select only 4 checkboxes 
		...
		let checkedChecks = document.querySelectorAll(".check:checked");
			if (checkedChecks.length === max ){
				...
				document.querySelectorAll('div.form-check  > input:not(.check:checked)').forEach((e) => {
					e.disabled = true;
				})
	...
	Microscope.getTrackStats = () => {
		...
		if (!Microscope.tracker && Microscope.stats.length) {
			...
			const timeSlices = app.splitTimeIntoChunks(Microscope.session.schedule, Microscope.session.ended_on, 6);
	...
	Microscope.datasets = (arr,uids) => {
		let datasets = []
		for (const key in Microscope.getName(uids)) {
			let data = {}
			if (Object.hasOwnProperty.call(Microscope.getName(uids), key)) {
				 let namesObj = Microscope.getName(uids)
				 data.label = namesObj[key];
				 data.data = Microscope.getDataFromUid(arr,key);
				 data.backgroundColor =  Microscope.colors[Math.floor(Math.random() * Microscope.colors.length)];
				
			}
			datasets.push(data)
		} 
		// subtracting previous character count from total character count in a single timeframe
		datasets.forEach((e ,i) => {
			...
			datasets[i].data = arrs;
			// console.log(datasets); console.log(arrs);
		})
		console.log("datasets rus right");
		return datasets
		
	}
	Microscope.getStudentName = (student) =>{
	...
	Microscope.getName = (uids) =>{
		let names = {}
		console.log(uids);
		uids.map(uid => {
			names[uid] = Microscope.getStudentName(window.students.find(e=>e.uid==uid))
		})
		console.log("get___ right");
		return names;
	}
	Microscope.getDataFromUid = (arr,uid) =>{
		let data = []
		arr.map(e=>{
			e.data.map(u=>{
				if(u.uid==uid){
	...
	Microscope.DrawThreadGraph = (uids = []) => {
		...
		console.log("Microscope thread stats",Microscope.threadStats);
		
		
		for (let key in Microscope.threadStats) {
			Microscope.threadStats[key] = Microscope.threadStats[key].reverse().filter(
				(elem => a =>
					(k => !elem[k] && (elem[k] = true))(a.uid + '|' + a.asset_type)
				)({})
			).filter((ele) => uids.length ? uids.indexOf(ele.uid) > -1 : !uids.length)
			
			//data.threads = [...data.threads, count]
			// console.log(Microscope.threadStats[key])
		}
		let arr = [];
		Object.keys(Microscope.threadStats).forEach((key) => {
			var result = [];
			Microscope.threadStats[key].reduce(function (res, value) {
				if (!res[value.uid]) {
					res[value.uid] = { uid: value.uid, count: 0 };
					result.push(res[value.uid])
				}
				res[value.uid].count = value.count.characters
				return res;
			}, {});
			arr.push({ time: key, data: result });
			
		})
		
		// console.log('arr',arr)
		// console.log('names',Microscope.getName(uids))
		
		
		// let datasets = arr.map(function (e,i){
		// 	let obj = {};
		// 	let user = (e) => window.students.find(u=>u.uid==e.uid);
		// 	obj.label = e.data.map(e => e.uid);
		// 	obj.data = e.data.map(e=>e.count);
		// 	obj.backgroundColor = e.data.map (e=> app.random.colors.random());
		// 	return obj;
		// })

		data.threads = data.threads.map((v, i) => i ? Math.abs(v - (data.threads[i - 1] || 0)) : v);

		let tracker = Microscope.$container.find('#graphmicroscope')[0];
		let labels = [];
		console.log(Microscope.statsGroup)
		if (!Microscope.tracker) {
		...
		Microscope.charts.threads = new Chart(tracker, {
			...
			data: {
				labels: labels,
				datasets: Microscope.datasets(arr,uids),
			},
			options: {
				indexAxis: 'x',
				onHover: function(e) {
					let obj = e.chart.tooltip.body;
					for (const property in obj) {
						Microscope.$container.find('#ms-details-text').text(obj[property].lines);
					  }
				},
				plugins: {
					legend: {
