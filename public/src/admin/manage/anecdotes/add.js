...
define('admin/manage/anecdotes/add', ['api'], function (api) {
	...
	add.init = function () {
		const _templates = {
			userInfo: function (data) {
				return `<div class="user-profile-picture">
							<img src="${data.picture}" alt="user-img" class="margin-right-3 object-cover square-85 border-radius-circle">
						</div>
		...
		$('body').on('click', '.user-listing', function () {
			...
			api.get(`/app/reflection?uid=${userDetails.uid}`, {}).then(function (response) {
				console.log(response);

				$('#table-body').empty();

				$.each(response.data, function (index, reflectionData) {
					const data = {
						index: index + 1,
						content: reflectionData.summary,
						uid: reflectionData.uid,
						displayname: reflectionData.author.username,
						userslug: reflectionData.author.userslug,
						imgsrc: reflectionData.author.picture,
					};
		...
		$('body').on('click', '.reflection-listing', function () {
			const listings = $('.reflection-listing');
		...
		$('body').on('click', '#submit-btn-car', function () {
			...
			if (placementLocs.length == 0) alert('Please pick where the anecdote has to be deployed');
			else {
				const payload = {
					title: $('#anecdoteTitle').text(),
					content: $('#anecdoteContent').text(),
