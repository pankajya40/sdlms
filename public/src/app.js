...
app.script = {
	HAMMERJS : {
		HAMMER : 'https://cdnjs.cloudflare.com/ajax/libs/hammer.js/2.0.8/hammer.js',
	}
}
app.link = {
	script: (url, callback) => {
...
app.notifyUser = (message, type='info') => {
	try {
		if (notify && typeof notify === 'function') {
			notify(message, type);
		}
	} catch (e) {
		console[type && type == 'error' ? 'error' : 'log'](message);
	}
}
...
const showSidePanel = config.sidePanelEnabledArray.some(item => window.location.pathname.includes(item));
...
(function () {
	...
	app.load = function () {
		...
		*/
		...
		if (config.sidePanelEnabledArray.some(item => window.location.pathname.includes(item))){
			require(['sdlms/sidePanel'], () => {
				new SidePanel();
			});
		}
	...
	function registerServiceWorker() {
		if ('serviceWorker' in navigator) {
			...
			// 		console.info('ServiceWorker registration succeeded.',registeration);
			// 	})
			// 	.catch(function (err) {
	...
	$('body').on('click', '[collpsible] [collapse]', function (e) {
		...
		$(this).parents('[collpsible]').first().find('[collapse-icon]')
			.toggleClass('rotate');
		$(this).find('[collapse-icon]').toggleClass('flip');
	});
	...
	((() => {
		...
		$(document).on('click', '#install-button', function (e) {
			if (window.matchMedia('(display-mode: browser)').matches && deferredPrompt) {
				...
				deferredPrompt.userChoice.then((choiceResult) => {
					if (choiceResult.outcome === 'accepted') {
						...
						notify('App Install Cancelled', 'error');
					}
					deferredPrompt = null;
				})
			}
	...
	app.install = () => {
		$('#install-button').remove();
		$('body').append('<div id="install-button" ></div>');
		$('#install-button').trigger('click');
	};

	app.whitelisted =  function(url){
		return ['http://localhost:4567'].some(restricted => url.includes(restricted));
	};
	...
	app.script = {
		JQUERY : {
			MOBILE : 'https://ajax.googleapis.com/ajax/libs/jquerymobile/1.4.5/jquery.mobile.js'
		}
	}
	app.link = {
		script: (url, callback) => {
	...
	// Highlighting text Popup Appears to reason(rigor builder) or feed
	(function ($) {
		if(config.sidePanelEnabledArray.some(item => window.location.pathname.includes(item))){

			$.fn.goshare = function () {
				$('body')
					.append(`<div id="social" class="px-3 py-2 rounded-lg button-secondary text-white" style="z-index:1000; position: absolute; display:none; -webkit-transition: transition: opacity 0.8s ease; -moz-transition: opacity 0.8s ease;">
			<a id="rigorBuilder" style="float: left; margin-right: 4px; display: block; cursor: pointer; padding-right: 6px; border-right: 2px solid #fff;"> Reason </a>
			<a id="maturity" style="float: left; margin-left: 4px; display: block; cursor: pointer;"> Maturity </a>
			<a id="insight" style="float: left; margin-left: 4px; display: block; cursor: pointer; padding-left:6px; border-left: 2px solid #fff"> Insight </a>
			</div>`);

				// adding event listener in reason btn
				$('#rigorBuilder')
					.mousedown(function (e) {
						let selectionText = window.getSelection && window.getSelection().type === 'Range' ? window.getSelection()
							.toString() : 'no statement';
							new RigorMini(selectionText, e);
					});
				$('#insight')
					.mousedown(function (e) {
						let selectionText = window.getSelection && window.getSelection().type === 'Range' ? window.getSelection().toString() : 'no statement';
						new InsightSpotter(selectionText, e);
					});
				$('#maturity')
					.mousedown(function (e) {
						let selectionText = window.getSelection && window.getSelection().type === 'Range' ? window.getSelection()
							.toString() : 'no statement';
							new MaturityMini(selectionText, e);
					});
				$(document)
					.ready(function () {
						$(this)
							.mouseup(function (e) {
								const getTool = document.getElementById('social');
								s = window.getSelection();
								oRange = s.getRangeAt(0); //get the text range
								oRect = oRange.getBoundingClientRect();
								s.toString().length
									? ((getTool.style.left = (e.pageX - 50) + 'px'),
										(getTool.style.top = (e.pageY - 50) + 'px'),
										getTool.style.display = 'block')
