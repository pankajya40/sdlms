		...
		$target.append(
			...
			.append($that.data.action != 'reader' ?
				$("<form>").attr({
					id: "form-" + $that.id,
					novalidate: true,
					class: 'row sdlms-article-container border create p-3 sdlms-article-container needs-validation '+ ($that.data.classes ? $that.data.classes : '') + ($that.data.action == 'reader' ? ' readonly' : ' create'),
				}).css({
