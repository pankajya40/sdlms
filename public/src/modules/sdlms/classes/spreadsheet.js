 ...
 */
 ...
 class spreadSheet {
	...
	create(data = null) {
		...
		console.log('Spread sheet',data.data);
		$that.spreadsheet = jspreadsheet($target[0], {
			...
			onchange: function (instance, cell, x, y, value) {
				if (!$that.data.noEvents && $that.data.isBrodcaster && !$that.data.isListener) {
					...
					socket.emit('sdlms.class.assets.spreadsheet.update', data, (err) => {
						console.log(err)
					});
				}
			...
			oninsertcolumn: function (instance, x, y) {
				if (!$that.data.noEvents && $that.data.isBrodcaster && !$that.data.isListener) {
					socket.emit('sdlms.class.assets.spreadsheet.update', {
						event: 'column_added',
						x: x,
						y: y,
						tid: $that.tid
		...
		if ($that.data.addFeedbacks && $that.data.id) {
			if (typeof FeedBacks !== 'function') {
				return console.log("SP:: Please add Feedbacks JS.");
			}
		...
		$(window).on('beforeunload', () => {$that.draft();});
	...
	reader(data = null) {
		...
		// data.data 🙄 this is bad but somehow this is the only way to get the data, Need to updated in V2
		data.data = data.data;

		data.readonly = app.isParsableJSON(data.readonly) ? JSON.parse(data.readonly) : data.readonly;
		data.widths = app.isParsableJSON(data.widths) ? JSON.parse(data.widths) : data.widths;
		console.log(data);
		$that.spreadsheet = jspreadsheet($target[0], { //$that.spreadsheet = tiui.cal($target[0], {
			minDimensions: [16, 16],
			data: data.data,
			updateTable: function (el, cell, x, y, source, value, id) {
				cell.classList.add('readonly');
		...
		if (!$that.data.queue) {
			...
			require(['api'], function (api) {
				...
				request.then(r => {
					...
					$that.data.id = r.pid;
					r.lookup_id = $that.lookup_id || null ;
					r.type = 'spreadsheet';
					resolve(r);
					if ($that.data.reload) {
						location.reload();
