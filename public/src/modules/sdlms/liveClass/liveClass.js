...
define("sdlms/liveClass/liveClass", [
	...
	LIVE_CLASS.usersCount = session.attendance.length || 1;
	LIVE_CLASS.reactionsCounter = {};
	LIVE_CLASS.reactionsArray = [];
	LIVE_CLASS.QuizInstance = null;

	// Maximum number of reactions to count in a particular timeslot, as of now it's 20% of total users
	LIVE_CLASS.MAX_REACTIONS = Math.ceil((20 / 100) * LIVE_CLASS.usersCount);
	LIVE_CLASS.BROADCAST_AFTER = 2 * (60 * 1000); // Time in milliseconds after which the broadcast is considered to be done
	...
	LIVE_CLASS.init = (data) => {
		...
		app.addSessionDetails(
			session.schedule,
			"sdlms-session-schedule",
			session.ended_on
		);
		$(".sdlms-container")
			.find(".sdlms-calender-date")
			.append(`${moment(session.schedule).format("DD")}`);
		$(".sdlms-container")
			.find(".sdlms-calender-month")
			.append(`${moment(session.schedule).format("MMM")}`);

		let checkLoading = setInterval(() => {
			if (LIVE_CLASS.memberLoaded) {
				app.loader(0);
				clearInterval(checkLoading);
				LIVE_CLASS.initSocialFeed();
				LIVE_CLASS.initPolls();
			}
		}, 1000);

		// setTimeout(() => {
		// 	LIVE_CLASS.broadcastReactions({broadcast: true});
		// }, LIVE_CLASS.BROADCAST_AFTER)
	};
	LIVE_CLASS.broadcastReactions = (data = {}) => {
		if (data.id) {
			if (LIVE_CLASS.reactionsCounter[data.id]) {
				LIVE_CLASS.reactionsCounter[data.id] += 1;
			} else {
				LIVE_CLASS.reactionsCounter[data.id] = 1;
			}
			...
			reactions.push({ ...data, timestamp: Date.now() });
			localStorage.setItem("reactions", JSON.stringify(reactions));
		}

		if (data.broadcast) {
			for (const reaction in LIVE_CLASS.reactionsCounter) {
				if (LIVE_CLASS.reactionsCounter[reaction] >= LIVE_CLASS.MAX_REACTIONS) {
					let highlightedReaction = LIVE_CLASS.reactionsArray.find(
						(elem) => elem.id == reaction
					);
					// We can call our Broadcast function here that would render the animation
					console.log(highlightedReaction);
				}
			}
			LIVE_CLASS.reactionsCounter = {};
		}
	...
	LIVE_CLASS.initMembers = () => {
		api
			.get(`/sdlms/${tid}/attendance`, {
				limit: 50,
			})
			.then((r) => {
				$("body")
					.find(".sdlms-asset-selection-user-body")
					.append(
						LIVE_CLASS._template(
							"studentSearch",
							$.extend({}, session.teacher, {
								role: "teacher",
				...
				$.each(r.data, function (index, student) {
					try {
						LIVE_CLASS.QuizInstance.addUidForAllocation(student.uid);
					} catch (error) {
						
					}
					$("body")
						.find(".sdlms-section-list")
						.append(
							LIVE_CLASS._template(
								"student",
								$.extend({}, student, {
									role:
										session.teacher.uid == student.uid ? "teacher" : "student",
					...
					if (
						...
						window.students.indexOf(student.uid) === -1
					)
						window.students.push(student);
					$("body")
						.find(".sdlms-asset-selection-user-body")
						.append(app._template("studentSearch", student));
				});

				$("body")
					.find(".sdlms-asset-selection-user-body")
					.off("click")
					.on("click", "[data-students-search]", function (e) {
						...
						if (member.uid) {
							...
							$("#studentAssetView").removeClass(
								"h-100 w-100 text-center d-flex justify-content-center align-items-center"
							);
							$("[sdlms-members-asset-view],[sdlms-search]").toggleClass(
								"w-100 sdlms-w-0"
							);
							$("[asset-selection-label]").text(
								`${member.fullname || member.displayname || member.username
								}'s Eagle Builder`
							);
							...
							if (
								(!ajaxify.data.tracker && member.role == "teacher") ||
								member.role != "teacher"
							) {
								...
								api
									.get(`/sdlms/${tid}/eaglebuilder?uid=${member.uid}`, {})
									.then((r) => {
										const data = {
											meta: r.meta,
											tracks: r.tracks,
	...
	LIVE_CLASS._template = (part, data = {}) => {
		const components = {
			student: () => {
				...
				return ` 
                <li class="d-flex align-items-center" user-data user-uid="${data.uid
					}" data-title="${data.fullname || data.displayname || data.username
					}"  title-bottom>
                <div class="col-8 d-flex pl-0" user-info>
                    <div class="user-img position-relative" data-reacted="${data.uid
					}">
                        <img onerror="${app.IMG_ERROR()}" src="${data.picture
					}" alt="" />
                    </div>
                    <div class="user-info pl-2 d-flex flex-column">
                        ...
                        <div class="user-last-activity hidden-on-collapsed sdlms-sub-text-primary-12px font-weight-500" auto-update-time-ago="${data.stats ? data.stats.eb.timestamp : 0
					}">${moment(
						data.stats ? data.stats.eb.timestamp : 0
					).fromNow()}</div>
                    </div>
                </div>
                ...
                <div class="col-2 hidden-on-collapsed px-0 d-flex justify-content-end align-items-center text-right">
					...
					<span eb>${data.stats &&
						data.stats.eb.count &&
						data.stats.eb.count.characters > 0
						? data.stats.eb.count.characters
						: 0
					} L</span>&nbsp;|&nbsp;<span tb>${data.stats &&
						data.stats.tb.count &&
						data.stats.tb.count.characters > 0
						? data.stats.tb.count.characters
						: 0
					} L</span>
                    </div>
                </div>
			...
			studentSearch:
				() => ` <div class="sdlms-assets-selection-user-list" data-students-search data-fullname="${data.fullname
					}" data-displayname="${data.displayname}" data-username="${data.username
					}" data-uid="${data.uid}" data-is-teacher="${data.isTeacher
					}" data-role="${data.role}" >
				<div class="col-11 mx-auto">
					<div class="d-flex align-items-center py-2 justify-content-start">
						<img onerror="${app.IMG_ERROR()}" src="${data.picture
					}" class="rounded-circle" alt="" /><span class="sdlms-text-tertiary-16px text-ellipse font-weight-medium ml-3">${data.fullname || data.displayname || data.username
					} ${data.uid == uid ? `(${data.isTeacher ? "Teacher, " : ""}Me)` : ""
					}</span>
					</div>
				</div>
			</div>`,
	...
	LIVE_CLASS.track = () => {
		...
		const sessionInterval = setInterval(() => {
			var now = Date.now();
			var percentage = (
				(Math.abs(now - start) / Math.abs(end - start)) *
				100
			).toFixed(2);
			$container.find('[sdlms-session="tracker"]').css({
				width: `${percentage >= 100 ? 100 : percentage}%`,
				background: `linear-gradient(to right, var(--secondary-background-color) ${percentage <= 100 ? percentage : 0
					}%, ${percentage > 100 ? "var(--primary-danger-color)" : "transparent"
					} ${percentage > 100 ? 0 : 0}%)`,
				borderRadius: `calc(var(--primary-border-radius)/1.8) ${percentage >= 100 ? "calc(var(--primary-border-radius)/1.8)" : 0
					}  ${percentage >= 100 ? "calc(var(--primary-border-radius)/1.8)" : 0
					} calc(var(--primary-border-radius)/1.8)`,
			});
			$container.find('[sdlms-session="timer"]').text(`${percentage}`);
			$container
				.find(".session-tracker-completion")
			[percentage >= 100 ? "addClass" : "removeClass"](
				"sdlms-session-tracker-completion-warning"
			);
		}, 1000);

		// session slider mobile
		$(".touch-icon").click(() => {
		...
		if (ajaxify.data.isTeacher) {
			...
			$container
				...
				.on("click", function () {
					if (
						$(this).attr("sdlms-session") != "stop" ||
						confirm("Stop Current Session?")
					) {
						const state = $(this).attr("sdlms-session");
						updateState(state);
						socket.emit(
							"meta.live.stateChange",
							$.extend(
								{},
								{},
								{
			...
			socket.on("meta.live.stateChange", (data) => {
				if (data.tid == tid) {
					notify(
						`Session has been ${$(`[sdlms-session="${data.event}"]`)
							.parents(".tracker-state")
							.find(".tracker-state-label")
							.data("textActive")}`,
						"info",
						5000
					);
	...
	LIVE_CLASS.getAsset = async (uid, type) => {
		if (type == "tb") {
			...
			const userSP = await api.get(
				`/sdlms/spreadsheet?tid=${tid}&uid=${uid}`,
				{}
			);
			...
			if (!ajaxify.data.isTeacher && ajaxify.data.isStudent) {
				teacherSP = await api.get(
					`/sdlms/spreadsheet?tid=${tid}&uid=${session.teacher.uid}`,
					{}
				);
	...
	LIVE_CLASS.reactions = () => {
		function react(data) {
			$(`[data-reacted="${data.uid}"]`).append(
				`<img onerror="${app.IMG_ERROR()}" class="user-reacted" src="${data.icon
				}" alt="">`
			);
			setTimeout(() => {
		...
		socket.on("meta.live.reaction", (data) => {
			...
			if (tid == data.tid) {
				...
				if (ajaxify.data.isTeacher) {
					notify(
						`${data.fullname || data.displayname || data.username
						} reacted <img onerror="${app.IMG_ERROR()}" style="width:24px;height:24px;border-radius:50%" class="ml-2" src="${data.icon
						}">`,
						"info",
						5000
					);
	...
	LIVE_CLASS.events = () => {
		try {
			if (location.search.includes("share_redirect")) {
				...
				history.pushState(
					null,
					"",
					location.href.replaceAll("share_redirect", "")
				);
		...
		socket.on("event:class.assets.update", (data) => {
			if (data.latest) {
				$(`[user-data][user-uid="${data.uid}"]`)
					.find(".user-last-activity ")
					.html(`Last Minute`);
				$(`[user-data][user-uid="${data.uid}"]`)
					.find(`[${data.asset_type}]`)
					.html(
						`${data.latest.count && data.latest.count.characters > 0
							? data.latest.count.characters
							: 0
						} L`
					);
			}
			...
			if (!Number(data.force_refresh) && data.value) {
				$("[members-assets]")
					.find(`[sdlms-id="${data.value.parent_id}"]`)
					.find(`[sdlms-id="${data.value.thread}"]`)
					.html(`${app.processString(data.value.value)}`);
			} else if (
				Number(data.force_refresh) &&
				$("[asset-selection-label].active").data("uid") == data.uid
			) {
		...
		socket.on("meta.live.feedback", (data) => {
			if (data.asset_owner_uid == uid && data.tid == tid) {
				const d = $(
					'[data-students-search][data-uid="' + data.creator + '"]'
				).data();
				notify(
					`${d.name || d.displayname || d.username} has given feedback on ${data.type == "eb" ? "Eagle Builder" : "Thread Builder"
					}`,
					"info",
					5000
				);
		...
		socket.on("event:class.joined", (data) => {
			...
			try {
				LIVE_CLASS.QuizInstance.addUidForAllocation(data.uid);
			} catch (error) {
				
			}
			if (
				Array.isArray(window.students) &&
				window.students.indexOf(data.uid) === -1
			)
			...
			if (
				!$(".sdlms-asset-selection-user-body").find(
					`[data-students-search][data-uid="${data.uid}"]`
				).length
			) {
				$("body")
					.find(".sdlms-asset-selection-user-body")
					.append(app._template("studentSearch", data));
			}
		});
	...
	LIVE_CLASS.initPolls = () =>{
			...
			new Polls({
				target: "#SDLMSPolls",
				tid: tid,
				isStudent: ajaxify.data.isStudent,
				groups: [{ group: 'eureka', name: 'Eureka' }, { group: 'answer', name: 'Answer' }, { group: 'question', name: 'Question' }, { group: 'root', name: 'Root' }],
				statuses: [{
					value: 'new',
					name: 'New'
				},
			...
			// 	Polls.close();
			// });
			// socket.emit('sdlms.polls.getNonVoted',{
			// 	tid: tid,
			// 	group:'custom',
			// },function(err,data){
			// 	let poll = data.length ? data[0] : null;
			// 	if(poll) Polls.open([poll],tid);
			// });
		
	}

	LIVE_CLASS.initSocialFeed = function () {
		new SocialFeed({
				tid: session.tid,
				target: "#SDLMSSocialFeed",
				filters: {
					thoughts: [
						{
							icon: "https://blog.deepthought.education/wp-content/uploads/2022/08/sigma.png",
							type: '',
							title: "All",
						},
						{
							icon: "https://blog.deepthought.education/wp-content/uploads/2022/08/eureka.png",
							type: 'eureka',
							title: "Eureka",
						},
						{
							icon: "https://blog.deepthought.education/wp-content/uploads/2022/08/root.png",
							type: 'root',
							title: "Root",
						},
						{
							icon: "https://blog.deepthought.education/wp-content/uploads/2022/08/question.png",
							type: 'question',
							title: "Question",
						},
						{
							icon: "https://blog.deepthought.education/wp-content/uploads/2022/08/answer.png",
							type: 'answer',
							title: "Answer",
						}
					],
	...
	LIVE_CLASS.initQuiz = function () {
		LIVE_CLASS.QuizInstance = new Quiz(tid,{
			mode: session.teacher.uid == app.user.uid ? undefined : 'answer',
		});
		window.Quiz.listeners(LIVE_CLASS.QuizInstance);
		$('.session-quiz').off('click').on('click',function(){
			LIVE_CLASS.QuizInstance.state('show');
		});
	}
	return LIVE_CLASS;
});
