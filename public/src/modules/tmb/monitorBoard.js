...
define('tmb/monitorBoard', ['api', 'sdlms/eaglebuilder', 'sdlms/spreadsheet'], function (api, eb) {
	...
	$monitorBoard.instantSession = async() => {
		
		$('body').on('submit', '#createInstantSession', function (e) {
			e.preventDefault();
			let $form = $(this);
			let payload = {};
			payload.topic = $form.find('input[name="topic"]').val();
			payload.teaser = $form.find('input[name="teaser"]').val();
			payload.classCategoryId = 97;
			payload.batchCategoryId = 117;
			payload.schedule = 'Instant';
			payload.autoStart = true;
			payload.mode = 'public';
			payload.link_expiry = new Date().getTime() + 1000 * 60 * 60 * 24 * 7;
			payload.members = JSON.stringify([]);
			$(this).find('button[type="submit"]').prop('disabled', true);
			payload.ended_on = moment().add(1, 'hour').valueOf();
			api.post('/sdlms/monitor', payload).then((response) => {
				if (response.tid) notify('Session created successfully', 'success');
				response.sharer &&  response.sharer.uuid && app.copyText(location.origin + ('/sharer?id='+response.sharer.uuid))
				setTimeout(() => {
					location.pathname = `/live/${response.tid}`;
				}, 2000);
			}).catch((e) => {
		...
		// 		$createInstantSession.find('select[name="classCategoryId"]').append(`<option value="${e_.cid}">${e_.name}</option>`);
		// 	});

		// 	$createInstantSession.find('select[name="classCategoryId"]').on('change', function () {
		// 		let cid = $(this).val();
		// 		if (!cid) return;
		// 		let $batch = $createInstantSession.find('select[name="batchCategoryId"]');
		// 		$batch.html('');
		...
		// 	$createInstantSession.find('select[name="classCategoryId"]').val(highestCategory).trigger('change');
		// 	$createInstantSession.find('select[name="batchCategoryId"]').val(highestBatchCategory);
		// }).catch((e) => {

		// }).finally(() => {
		// 	$createInstantSession.removeClass('ajaxifying');
		// });
	}
	return $monitorBoard;
});
